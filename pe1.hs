...
makelisttogether [] = []
makelisttogether re = makelisttogether (tail(re)) ++ rest
    where rest = getrecipelist (head(re))
updatestock :: [(String,Double)]->[(String,Double)]->[(String,Double)]
updatestock l [] = l
updatestock st re = l1
    where 
        l1 = updatestock ust (tail re)
        ust = if length(filter(\x->getname x == getname (head re)) st)/=0 then map(\x->(updatestockhelp x (head re))) st else st++[(getname(head re),-1*getp(head re))]
updatestockhelp :: (String,Double) -> (String,Double) -> (String,Double)
updatestockhelp (a,b) (c,d) = (a,w)
    where w = if a == c then b-d else b
makeShoppingList :: [(String, Double)] -> [Recipe] -> [Price] -> [(String, Double, Double)]
makeShoppingList sto rec pri = sol
    where 
        sol = mslhelp (updatestock sto (makelisttogether rec)) pri 
mslhelp :: [(String, Double)] -> [Price] -> [(String, Double, Double)]
mslhelp [] _ = []
mslhelp sto pri = sol
    where 
        sol = l++(mslhelp (tail sto) pri)
        l = if getp (head sto)<0 then [(getname (head sto),getRounded(-1*(getp(head sto))),getRounded(getprice((filter(\x->getpricename x == getname(head sto))pri)!!0)*(-1*getp(head sto))))] else []